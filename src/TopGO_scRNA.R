...
if((InterestGenes == "up" &&  length(which(DE_results.df$avg_logFC > logFC & DE_results.df$p_val_adj < 0.01)) > 0) | (InterestGenes == "down" &&  length(which(DE_results.df$avg_logFC < -logFC & DE_results.df$p_val_adj < 0.01)) > 0) | (InterestGenes == "all" && length(which(abs(DE_results.df$avg_logFC) > logFC & DE_results.df$p_val_adj < 0.01)) > 0)){
...
# extract gene of interest
  if(InterestGenes == "up"){
  genesOfInterest <- rownames(DE_results.df[which(DE_results.df$avg_logFC > logFC & DE_results.df$p_val_adj < significant), ])
  }
  if(InterestGenes == "down"){
  genesOfInterest <- rownames(DE_results.df[which(DE_results.df$avg_logFC < -logFC & DE_results.df$p_val_adj < significant), ])
  }
  if(InterestGenes == "all"){
  genesOfInterest <- c(rownames(DE_results.df[which(DE_results.df$avg_logFC > logFC & DE_results.df$p_val_adj < significant), ]), rownames(DE_results.df[which(DE_results.df$avg_logFC < -logFC & DE_results.df$p_val_adj < significant), ]))}
